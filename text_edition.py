...
def analyze_and_extract_commands(user_input):
    ...
    while i < len(lines):
        ...
        condition_type = None  # Initialisation par défaut
        ...
        if "{" in line:  # Début d'un bloc
            ...
            if line.startswith("if"):
                condition_type = "if"
                condition_expression = line.split("(", 1)[1].split(")", 1)[0]  # Extraire la condition
...
def extract_command_data(tokens):
    ...
    if tokens[0][1] == 'create_cursor' and len(tokens) == 7:
        ...
        command_data = {
            'command': 'if',
            'condition': tokens[2][1],  # Condition extraite
            'block': []  # Le bloc sera géré dans `analyze_and_extract_commands`
        }
